...
class MetaPathGenerator:
    ...
    def read_data(self, theAns, numCluster):
        ...
        df_stu_ques = df_stu_ques[df_stu_ques['correct'] == theAns]
...
if __name__ == "__main__":
    ...
    # data_set = "ASSIST09"
    for data_set in ["Statics2011","Eedi","EdNet"]:
        ...
        for num_walks in [10]:
            for walk_length in [80]:
                for num_cluster in [120]:
                    out_file_name = os.path.join(save_folder, 'walks_qucuq_cw_%d_%d_%d.txt' % (
                        num_cluster, num_walks, walk_length))
                    outfile = open(out_file_name, 'w')
                    for ans in [0, 1]:
                        t = time.time()
                        mpg = MetaPathGenerator()
                        mpg.read_data(ans, num_cluster)
